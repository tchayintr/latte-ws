...
class LatticeBase(object):
    ...
    def add_edges(graph, directional: str = 'forward') -> nx.DiGraph:
        ...
        for i, u in enumerate(nodes):
            cur_node = nodes[u]
            # for v in list(nodes)[i + 1:]: # note: eos must be the last
    ...
    @staticmethod
    def pad_node_data(graph_org,
                      node_attr_name: str,
                      max_length: Optional[int] = None,
                      padding_value: int = 0):
        if not nx.get_node_attributes(graph_org, node_attr_name):
            raise IndexError

        graph = graph_org.copy(as_view=False)
        nodes = graph.nodes(data=node_attr_name)
        attrs = [node[1] for node in nodes]

        if max_length is None:
            max_length = len(max(attrs, key=len))

        padded_node = {}
        for idx, attrs in nodes:
            padded_node[idx] = attrs + ([padding_value] *
                                        (max_length - len(attrs)))
        nx.set_node_attributes(graph, padded_node, node_attr_name)
        return graph


class Lattice(LatticeBase):
    ...
    def __init__(self,
        ...
        self._build_graph_from_trie(tokens, trie, vocab, pad_token, unk_token,
                                    bos_token, eos_token, dataset_token,
                                    max_token_length, node_comp_type,
                                    rand_dropout, keep_unigram_node,
                                    directional)

    def _build_graph_from_trie(self, tokens: Union[str, List], trie: Trie,
                               vocab: Dict, pad_token: str, unk_token: str,
    ...
    def build_graph_from_trie(
            ...
            rand_dropout: float = 0.0,
            keep_unigram_node: bool = False,
            directional: str = 'bidirectional') -> nx.DiGraph:
        if isinstance(tokens, list):
            tokens = ''.join(tokens)
        token_len = len(tokens)

        token_id_lookup_fn = vocab.get
        ...
        for (end_idx, (insert_order, token)) in trie.iter(tokens):
            ...
            is_unk_token = token_id is None

            if ((is_unk_token and len(token) != 1) and
                (node_comp_type == 'avg' or node_comp_type == 'concat')):
                '''token_id list'''
                token_id = [token_id_lookup_fn(tok) for tok in list(token)]
            else:
                token_id = [token_id]
        ...
        # pad
        if (node_comp_type == 'avg' or node_comp_type == 'concat'):
            g = LatticeBase.pad_node_data(
                g,
                node_attr_name='token_id',
                max_length=max_token_length,
                padding_value=token_id_lookup_fn(pad_token))
...
class BertLattice(LatticeBase):
    ...
    def build_graph_from_trie(
        ...
        if node_comp_type == 'wv' and vocab is None:
            raise AssertionError('`node_comp_type=wv` requires `vocab`')

        token_id_lookup_fn = (vocab.get if (node_comp_type == 'wv'
                                            or node_comp_type == 'init') else
        ...
        for (end_idx, (insert_order, token)) in trie.iter(tokens):
            ...
            if (node_comp_type == 'avg' or node_comp_type == 'concat'):
                ...
                is_unk_token = BertLattice.is_unk_token(token_id,
                                                        tokenizer=tokenizer,
                                                        unk_token=unk_token)

            if (node_comp_type == 'avg' or node_comp_type
                    == 'concat') and (is_unk_token and len(token) != 1):
                '''token_id list'''
                token_id = token_id_lookup_fn(list(token))
            else:
                token_id = [token_id]
